...
class BoundMaxPool(BoundOptimizableActivation):
    ...
    def _init_opt_parameters_impl(self, size_spec, name_start):
        ...
        alpha = torch.empty(
            [1, size_spec, self.input_shape[0], self.input_shape[1], self.output_shape[-2], self.output_shape[-1], self.kernel_size[0], self.kernel_size[1]],
            dtype=torch.float, 
            device=ref.device, 
            requires_grad=True,
        )
    ...
    def bound_backward(self, last_lA, last_uA, x, start_node=None, unstable_idx=None, **kwargs):
        ...
        if paddings == (0,0,0,0):
            delete_upper = torch.scatter(torch.flatten(x.upper, -2),                  -1, torch.flatten(max_lower_index, -2), -torch.inf).view(upper_d.shape)
        else:
        ...
        if self.opt_stage == 'opt':
            ...
            alpha.data.copy_(self.project_simplex(alpha.data).clone().detach())
            ...
            if isinstance(last_A, torch.Tensor):
                ...
                def upsample(last_patches, last_A):
                    if last_A.unstable_idx is None:
                        patches = F.interpolate(last_patches.view(shape[0] * shape[1] * shape[2], *shape[3:]), scale_factor=[1,]+self.kernel_size)
                        patches = patches.view(shape[0], shape[1], shape[2], *patches.shape[1:])
                    else:
                        patches = F.interpolate(last_patches, scale_factor=[1,] + self.kernel_size)
                    return Patches(
                        patches=patches, 
                        stride=last_A.stride, 
                        padding=last_A.padding,
                ...
                padding, stride, output_padding = compute_patches_stride_padding(
                    ...
                    patches_padding=last_A.padding, 
                    patches_stride=last_A.stride, 
                ...
                next_A = Patches(
                    patches=next_A_patches, 
                    stride=stride, 
                    padding=padding, 
                    shape=next_A_patches.shape,
                    unstable_idx=last_A.unstable_idx, 
                    output_shape=last_A.output_shape,
                    inserted_zeros=last_A.inserted_zeros, 
    ...
    def bound_forward(self, dim_in, x):
        ...
        def _bound_oneside(w_pos, b_pos, w_neg, b_neg, d, b):
            ...
            w_pooled = F.avg_pool2d(w_new.view(-1, *w_new.shape[2:]), self.kernel_size, self.stride, self.padding, ceil_mode=self.ceil_mode) * tot_kernel_size
            w_pooled = w_pooled.reshape(w_new.shape[0], -1, *w_pooled.shape[1:])
            b_pooled = F.avg_pool2d(b_new, self.kernel_size, self.stride, self.padding, ceil_mode=self.ceil_mode) * tot_kernel_size + b
    ...
    def build_solver(self, *v, model, C=None, model_type="mip", solver_pkg="gurobi"):
        # e.g., last layer input gurobi vars (3,32,32), pre_layer_shape (1,32,27,27), this layer shape (1,32,6,6)
...
class BoundAveragePool(Bound):
    ...
    def bound_backward(self, last_lA, last_uA, x, **kwargs):
        def _bound_oneside(last_A):
            if last_A is None:
                return None, 0
            equal_kernel_stride = (self.kernel_size[0] == self.stride[0] and self.kernel_size[1] == self.stride[1])
            if isinstance(last_A, torch.Tensor):
                shape = last_A.size()
                if equal_kernel_stride:
                    # propagate A to the next layer, with batch concatenated together
                    next_A = F.interpolate(last_A.reshape(shape[0] * shape[1], *shape[2:]), scale_factor=self.kernel_size) / (prod(self.kernel_size))
                    ...
                    weight = torch.zeros(self.input_shape[1], self.output_shape[1], *self.kernel_size, dtype=last_A.dtype, device=last_A.device)
                    ...
                    output_padding0 = (int(self.input_shape[2]) - (int(self.output_shape[2]) - 1) * self.stride[0] + 2 * self.padding[0] - 1 - (int(weight.size()[2] - 1)))
                    output_padding1 = (int(self.input_shape[3]) - (int(self.output_shape[3]) - 1) * self.stride[1] + 2 * self.padding[1] - 1 - (int(weight.size()[3] - 1)))
                ...
                simplify_patch = (equal_kernel_stride and last_A.inserted_zeros + 1 == self.kernel_size[0] and self.kernel_size[0] == self.kernel_size[1])
                padding, stride, output_padding = compute_patches_stride_padding(
                    input_shape=self.input_shape, 
                    patches_padding=last_A.padding, 
                    patches_stride=last_A.stride,
