...
class BoundTwoPieceLinear(BoundOptimizableActivation):
    ...
    def init_opt_parameters(self, start_nodes):
        ...
        if (self.use_sparse_features_alpha and self.inputs[0].is_lower_bound_current() and self.inputs[0].is_upper_bound_current()):
            ...
            if self.alpha_indices[0].size(0) <= minimum_sparsity * total_neuron_size:
                ...
                if len(self.alpha_indices) == 1: # This layer is after a linear layer.
                    alpha_init = self.init_d[:, :, self.alpha_indices[0]]
                elif len(self.alpha_indices) == 3: # This layer is after a conv2d layer.
                    alpha_init = self.init_d[:, :, self.alpha_indices[0], self.alpha_indices[1], self.alpha_indices[2]]
                elif len(self.alpha_indices) == 2: # This layer is after a conv1d layer.
    ...
    def reconstruct_full_alpha(self, sparse_alpha, full_alpha_shape, alpha_indices):
        ...
        if len(alpha_indices) == 1: # Relu after a dense layer.
            full_alpha[:, :, alpha_indices[0]] = sparse_alpha
        elif len(alpha_indices) == 3: # Relu after a conv2d layer.
            full_alpha[:, :, alpha_indices[0], alpha_indices[1], alpha_indices[2]] = sparse_alpha
        elif len(alpha_indices) == 2: # Relu after a conv1d layer.
            full_alpha[:, :, alpha_indices[0], alpha_indices[1]] = sparse_alpha
        else:
            raise ValueError
        return full_alpha

    def bound_backward(self, last_lA, last_uA, x=None, start_node=None, unstable_idx=None, reduce_bias=True, **kwargs):
        """
        ...
        (upper_d, upper_b, lower_d, lower_b, lb_lower_d, ub_lower_d, lb_upper_d, ub_upper_d, lb_upper_b, ub_upper_b, alpha_lookup_idx) = self._backward_relaxation(last_lA, last_uA, x, start_node, unstable_idx)
...
class BoundRelu(BoundTwoPieceLinear):
    ...
    def get_unstable_idx(self):
        self.alpha_indices = torch.logical_and(self.inputs[0].lower < 0, self.inputs[0].upper > 0).any(dim=0).nonzero(as_tuple=True)
    ...
    def _relu_lower_bound_init(self, upper_k):
        ...
        if self.relu_options == "same-slope":
            ...
            lower_k = (upper_k > self.leaky_alpha).to(upper_k) + (upper_k <= self.leaky_alpha).to(upper_k) * self.leaky_alpha
        else:
            # adaptive
            if self.leaky_alpha == 0:
                ...
                lower_k = (upper_k > 0.5).to(upper_k) + (upper_k <= 0.5).to(upper_k) * self.leaky_alpha
        return lower_k
    ...
    def bound_dynamic_forward(self, x, max_dim=None, offset=0):
        ...
        if not hasattr(self, 'upper_k'):
            ...
            self.upper_k, self.upper_b = self._relu_upper_bound(x.lower, x.upper, self.leaky_alpha)
            self.upper_b /= 2
        ...
        if offset + w_new.shape[1] < x.tot_dim:
            return LinearBound(w_new, b_new, w_new, b_new, x_L=x.x_L, x_U=x.x_U, tot_dim=self.tot_dim)
        ...
        return LinearBound(w_new, b_new, w_new, b_new, x_L=x_L_new, x_U=x_U_new, tot_dim=self.tot_dim)
    ...
    @staticmethod
    def _relu_mask_alpha(lower, upper, lb_lower_d : Optional[Tensor], ub_lower_d : Optional[Tensor], leaky_alpha : float = 0):
    ...
    def _backward_relaxation(self, last_lA, last_uA, x, start_node, unstable_idx):
        ...
        if self.opt_stage in ['opt', 'reuse']:
            # Alpha-CROWN.
            lower_d = None
            selected_alpha, alpha_lookup_idx = self.select_alpha_by_idx(last_lA, last_uA, unstable_idx, start_node)
        ...
        for neuron_idx, pre_var in enumerate(gvars_array.reshape(-1)):
            ...
            if pre_lb >= 0:
                ...
                if model_type == "mip" or model_type == "lp_integer":
                    ...
                    new_relu_layer_constrs.append(model.addConstr(var >= pre_var, name=f'ReLU{self.name}_{neuron_idx}_a_1'))
                    new_relu_layer_constrs.append(model.addConstr(pre_ub * a >= var, name=f'ReLU{self.name}_{neuron_idx}_a_2'))

                elif model_type == "lp":
                    new_relu_layer_constrs.append(model.addConstr(var >= pre_var, name=f'ReLU{self.name}_{neuron_idx}_a_0'))
                    new_relu_layer_constrs.append(model.addConstr(pre_ub * pre_var - (pre_ub - pre_lb) * var >= pre_ub * pre_lb, name=f'ReLU{self.name}_{neuron_idx}_a_1'))
...
class BoundSign(BoundActivation):
    ...
    def bound_relax(self, x, init=False):
        ...
        mask_pos = x.lower > 0
        mask_neg = x.upper < 0
        mask_both = torch.logical_not(torch.logical_or(torch.logical_or(mask_0, torch.logical_or(mask_pos, mask_pos_0)), torch.logical_or(mask_neg, mask_neg_0)))
        self.add_linear_relaxation(mask=mask_0,                                 type='lower', k=0,                           x0=torch.zeros_like(x.upper, requires_grad=True), y0=0)
        self.add_linear_relaxation(mask=mask_0,                                 type='upper', k=0,                           x0=torch.zeros_like(x.upper, requires_grad=True), y0=0)
        
        self.add_linear_relaxation(mask=mask_pos_0,                             type='lower', k=1/x.upper.clamp(min=1e-8),   x0=torch.zeros_like(x.upper),                     y0=0)
        self.add_linear_relaxation(mask=torch.logical_or(mask_pos_0, mask_pos), type='upper', k=0,                           x0=torch.zeros_like(x.upper, requires_grad=True), y0=1)
        
        self.add_linear_relaxation(mask=torch.logical_or(mask_neg_0, mask_neg), type='lower', k=0,                           x0=torch.zeros_like(x.upper, requires_grad=True), y0=-1)
        self.add_linear_relaxation(mask=mask_neg_0,                             type='upper', k=-1/x.lower.clamp(max=-1e-8), x0=torch.zeros_like(x.upper),                     y0=0)
    ...
    def _backward_relaxation(self, last_lA, last_uA, x, start_node, unstable_idx):
        ...
        if self.opt_stage in ['opt', 'reuse']:
            ...
            if self.alpha_indices is not None:
                ...
                if lb_lower_d is not None:
                    lb_lower_d = self.reconstruct_full_alpha(lb_lower_d, full_alpha_shape, self.alpha_indices)
                    lb_upper_d = self.reconstruct_full_alpha(lb_upper_d, full_alpha_shape, self.alpha_indices)
                if ub_lower_d is not None:
                    ub_lower_d = self.reconstruct_full_alpha(ub_lower_d, full_alpha_shape, self.alpha_indices)
                    ub_upper_d = self.reconstruct_full_alpha(ub_upper_d, full_alpha_shape, self.alpha_indices)

            lb_lower_d, ub_lower_d, lb_upper_d, ub_upper_d = self._mask_alpha(lower, upper, lb_lower_d, ub_lower_d, lb_upper_d, ub_upper_d)
            flag_expand = True  # we already have the spec dimension.
        else:
        ...
        if not flag_expand:
            if self.opt_stage in ['opt', 'reuse']:
                ...
                lower_d = lower_d.unsqueeze(0)
                upper_d = upper_d.unsqueeze(0)
        return (upper_d, upper_b, lower_d, lower_b, lb_lower_d, ub_lower_d, lb_upper_d, ub_upper_d, None, None, alpha_lookup_idx)
    ...
    def build_solver(self, *v, model, C=None, model_type="mip", solver_pkg="gurobi"):
        ...
        for neuron_idx, pre_var in enumerate(gvars_array.reshape(-1)):
            ...
            if pre_lb >= 0:
                ...
                layer_constrs.append(model.addConstr(var == 1 - 2*a, name=f'Sign{self.name}_{neuron_idx}_a_2'))
